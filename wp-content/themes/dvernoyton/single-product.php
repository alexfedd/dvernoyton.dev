          ...
          <div class="product-banner__selection">
            ...
            <div class="product-banner__selectors">
              ...
              foreach ( $nal_terms as $index => $term ) :
                  ...
                  // raw cost from the cost‑terms
                  $cost_raw = '';
                  if ( ! empty( $cost_terms ) && isset( $cost_terms[ $index ] ) ) {
                      $cost_raw = trim( $cost_terms[ $index ]->name );
                  }

                  // format with spaces every 3 digits
                  $cost_formatted = $cost_raw !== ''
                      ? number_format( (int) $cost_raw, 0, '', ' ' )
                      : '';

                  // build the display name
                  $display_name = $term->name;
                  if ( $cost_formatted !== '' ) {
                      $display_name .= ' (+' . $cost_formatted . ' р.)';
                  }
              ?>
                  <label for="nal_<?php echo esc_attr( $term->slug ); ?>" class="product-banner__label">
                      <input 
                          ...
                          id="nal_<?php echo esc_attr( $term->slug ); ?>" 
                          <?php if ( $term === reset( $nal_terms ) ) echo 'checked'; ?> 
                      />
                      <span class="product-banner__label-text">
